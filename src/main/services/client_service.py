...
class ClientService:
    ...
    async def get_all_clients(self, page) -> PageResponse[Client]:
        ...
        result = await ClientRepository.get_all(self.session, self.customer, (page-1)*self.page_size, self.page_size)
        response = [
            Client.model_validate(client, from_attributes=True)
