...
class SupplierService:
    ...
    async def get_all_suppliers(self, page) -> PageResponse[Supplier]:
        ...
        result = await SupplierRepository.get_all(self.session, self.customer, (page-1)*self.page_size, self.page_size)
        response = [
            Supplier.model_validate(supplier, from_attributes=True)
