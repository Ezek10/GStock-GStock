...
class SellerService:
    ...
    async def get_all_sellers(self, page) -> PageResponse[Seller]:
        ...
        result = await SellerRepository.get_all(self.session, self.customer, (page-1)*self.page_size, self.page_size)
        response = [
            Seller.model_validate(seller, from_attributes=True)
