...
async def get_products(request: Request, session: AsyncSession = Depends(get_db_session), page: int = 1):
...
async def get_products_stocks(request: Request, session: AsyncSession = Depends(get_db_session), page: int = 1):
